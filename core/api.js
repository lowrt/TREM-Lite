...
async function refresh_report_list(_fetch = false, data = {}) {
	...
	for (let i = (IsPalert) ? -1 : 0; i < report_data.length; i++) {
		...
		if (i == -1)
			...
			if (i == 0 && !IsPalert)
				report.innerHTML = `<div class="report"><div id="${originTime}_info" class="report_item"><div class="report_text report_intensity intensity_${intensity}"style="font-size: ${(resize) ? "50" : "60"}px;">${intensity_level}</div><div class="report_text_box"><div class="report_text" style="font-size: 22px;"><b>${loc}</b></div><div class="report_text" style="font-size: 15px;">${time}</div><div style="display: flex;"><div class="report_text"><b>M&nbsp;${report_data[i].magnitudeValue.toFixed(1)}</b></div><div class="report_text report_scale" style="width: 100%;text-align: right;">深度:&nbsp;<b>${report_data[i].depth}</b>&nbsp;km</div></div></div></div><div id="${originTime}_click_box" class="report_click hide"><i id="${originTime}_click_replay" style="color: red;" class="report_click_text fa-regular fa-circle-play fa-2x"></i><i id="${originTime}_click_web" style="${report_data[i].location.startsWith("TREM 人工定位") ? "color: red;" : ""}" class="report_click_text fa fa-globe fa-2x"></i></div></div>`;
			else
